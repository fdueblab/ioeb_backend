...
# 定义代码审查响应模型
code_review_response = api.model('CodeReviewResponse', {
    'status': fields.String(description='响应状态', default='fail'),
    'message': fields.String(description='错误消息'),
    ...
    def post(self):
        ...
        try:
            ...
            for temp_dir in temp_dirs:
                # 跳过包含输出ZIP文件的目录
                if output_zip and os.path.commonpath([temp_dir]) == os.path.commonpath([os.path.dirname(output_zip)]):
                    continue
                try:
                    cleanup(temp_dir)
                except Exception as e:
